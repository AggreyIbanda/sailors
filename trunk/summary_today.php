...
<table>
    <!--   bar -->
    <tr><th colspan="4">Bar Sales Today (<?php echo $date_today; ?>)</th></tr>
    <tr><th>Date</th><th>total Sales(Ksh)</th><th>Total Cost(Ksh)</th><th>Profit</th></tr>
    <?php
    
    $sql = "SELECT * FROM bar where transactionDate = '$date_today' order by id";
    ...
    <table>
        <!-- butchery       -->
        <tr><th colspan="5">Butchery  Sales Today (<?php echo $date_today; ?>)</th></tr>
        <tr><th>Date</th><th>Waste(Kg)</th><th>total Sales(Ksh)</th><th>Total Cost(Ksh)</th><th>Profit</th></tr>
    <?php
        $sql = "SELECT * FROM butchery where transactionDate = '$date_today'  order by id";
        $result = mysql_query($sql) or die("Error executing '$sql':" . mysql_error());
        while ($row = mysql_fetch_assoc($result)) {
            extract($row);
    ?>
    ...
    <table>
        <!--    rooms    -->
        <tr><th colspan="4">Occupied Rooms  Today (<?php echo $date_today; ?>)</th></tr>
        <tr><th>Date</th><th>Self Contained</th><th>Single</th><th>Total Revenue(Ksh)</th></tr>
    <?php
        $sql = "SELECT * FROM accommodation  where dateOccupied = '$date_today'   order by id";
        $result = mysql_query($sql) or die("Error executing '$sql':" . mysql_error());
        $single = 0;
        $self = 0;
        while ($row = mysql_fetch_assoc($result)) {
